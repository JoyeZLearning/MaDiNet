...
if mode == 'train':
    ...
    img_path = 'val2017'
# elif mode == 'test':
#     anno_path = 'SAR_Ship_dataset/annotations/image_info_test2017.json'
#     img_path = 'test2017'
else:
...
coco_dets = coco.COCO('/DiffusionDet-main/output_RE/inference/coco_eval_instances_results.json')
...
# show test_gt
    file_path = 'D:/softapp/Anaconda/envs/pytorch_gpu/DiffusionDet-main/datasets/val_results'
    file_path = os.path.join(file_path,str(img_id)+'.jpg')
    frame = cv2_demo_det_gt(img.copy(),det_coords,coords[:,:])
    # frame = cv2_demo_gt(img.copy(),coords[:,:])
    # cv2.imshow('frame_det',frame)
    # cv2.waitKey(1000)
    # print(file_path)
